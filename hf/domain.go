...
// Domain also builds the standalone hf binary (see cli.NewApp), so the
...
func (Domain) Info() kit.DomainInfo {
	return kit.DomainInfo{
		...
		Identity: kit.Identity{
			...
			Long: `A command line for Hugging Face.
...
func (Domain) Register(app *kit.App) {
	...
	// Resolver op: one record per id, the home of `hf page` and
	// `ant get hf://page/<id>`.
	kit.Handle(app, kit.OpMeta{Name: "page", Group: "read", Single: true,
		Summary: "Fetch a page by path or URL", URIType: "page", Resolver: true,
		Args: []kit.Arg{{Name: "ref", Help: "page path or URL"}}}, getPage)

	// List op: members of a page, the home of `hf links` and `ant ls`.
	// It emits page stubs, so every listed member is itself an addressable
	// hf://page/ URI a host can follow.
...
// Classify turns any accepted input — a bare path or a full hf.com URL —
...
func (Domain) Locate(uriType, id string) (string, error) {
	if uriType != "page" {
		return "", errs.Usage("hf has no resource type %q", uriType)
	}
